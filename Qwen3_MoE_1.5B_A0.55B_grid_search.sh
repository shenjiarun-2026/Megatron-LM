...
# Default coarse grid: tune only LR and WD first
LR_GRID_STAGE1=(${LR_GRID_STAGE1:-0.5e-2 0.8e-2 1.0e-2 1.2e-2})
WD_GRID_STAGE1=(${WD_GRID_STAGE1:-0.1})
MUON_EXTRA_SCALE_GRID_STAGE1=(${MUON_EXTRA_SCALE_GRID_STAGE1:-1.0})

# Stage 2 refinement around chosen best LR/WD
BEST_LR=${BEST_LR:-8e-4}
BEST_WD=${BEST_WD:-0.10}
MUON_EXTRA_SCALE_GRID_STAGE2=(${MUON_EXTRA_SCALE_GRID_STAGE2:-0.85 1.0 1.15})
...
run_one() {
    ...
    local log_name
    log_name="Qwen3MoE1p5B_${OPTIMIZER_TYPE}_lr${current_lr}_wd${current_wd}_TP_SIZE_${TP_SIZE}"
